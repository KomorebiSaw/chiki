...
class NGH(Base):
    ...
    HOST = 'pay.neargh.com'
    CALLBACK_HOST = ''
    PREPAY_URL = '/paying/weifutong/getCodeUrl'
    QUERY_URL = '/paying/weifutong/getPayState'

    def __init__(self, app=None, key=None, config=None, holder=None):
        self.callback = None
        super(NGH, self).__init__(app, key, config, holder)

    def init_app(self, app):
        super(NGH, self).init_app(app)
        ...
        self.callback_url = self.get_config(
            'callback_url', '/callback/ngh/[key]/')
        self.endpoint = self.get_config(
            'endpoint', 'ngh_[key]_callback')
